...
cc.Class({
    ...
    onLoad: function () {
        ...
        this.radius_BG = this.node.width / 2;
        // joy下的Game组件
    ...
    _getRadian: function (point) {
        return  Math.PI / 180 * this._getAngle(point);
        return this._radian;
    },
    ...
    _touchMoveEvent: function (event) {
        // var touchPos = this.node.convertToNodeSpaceAR(event.getLocation());
        // // 背景圆，不能超出背景圆
        // var clone_positsion=touchPos.clone();
        // if (touchPos.x > this.radius_BG || touchPos.x < -this.radius_BG)
        // touchPos.x = touchPos.x / Math.abs(touchPos.x) * this.radius_BG;
        // if (touchPos.y> this.radius_BG || touchPos.y < -this.radius_BG)
        // touchPos.y = touchPos.y / Math.abs(touchPos.y) * this.radius_BG;

        // this.dot.setPosition(touchPos);
        var touchPos = this.node.convertToNodeSpaceAR(event.getLocation());
        var distance = this._getDistance(touchPos,cc.v2(0,0));
        var radius = this.radius_BG;
        ...
        else
        {
            ...
            var x =  Math.cos(this._getRadian(touchPos)) * radius;
            var y =  Math.sin(this._getRadian(touchPos)) * radius;
